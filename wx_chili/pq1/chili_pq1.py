...
# 获取当前文件夹名称（如 pq1, pq2 等）
current_folder = os.path.basename(os.getcwd())
region_id = int(current_folder.replace('pq', ''))  # 提取片区编号（如 pq1 -> 1）
...
try:
    with connection.cursor() as cursor:
        # 查询当前片区数据
        sql = f"SELECT * FROM chili_1pq WHERE id = {region_id}"
        cursor.execute(sql)
        record = cursor.fetchone()

        # 提取记录中的关键信息
        region_name = record['region_name'] if record else f'第{region_id}片区'
        ...
        qr.add_data(f"http://110.41.56.99/{current_folder}/{html_file_name}")
        qr.make(fit=True)
        qr_img = qr.make_image(fill_color="black", back_color="white")
        qr_file_name = f"{['first', 'second', 'third', 'fourth', 'fifth', 'sixth'][region_id - 1]}_chili_qr.png"
